<template>
  ...
  >
      ...
      <el-submenu :index="item.path" v-if="item.children" :key="item.path">
        ...
        <template v-for="uu in item.children">
          <el-menu-item
            ...
            v-if="
              (uu.deviceType &&
                uu.deviceType.includes(currentDevice.deviceTypeId)) ||
                !uu.deviceType
            "
    ...
    <el-menu-item class="menu-index" index="/index" key="index">{{
      $translate("首页")
    }}</el-menu-item>
  </el-menu>
</template>
...
export default {
  ...
  mounted() {
    ...
    let { deviceTypeId } = this.currentDevice;
    let { children } = this.$router.options.routes.find(
      item => item.deviceType && item.deviceType.includes(deviceTypeId)
    );
    let systemRoutes = children.find(item => item.path === "/system");
    this.routes = systemRoutes.children;
    this.activeIndex = this.$route.path;
    console.log(this.devicePermissions);
  },
  computed: {
